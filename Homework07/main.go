...
func main() {
	...
	// Канал для передачи объектов (игр, игроков, ходов) в хранилище
	...
	go func() {
		for {
			...
			if len(активные) > 0 {
				// Определяем ширину доски по первой активной партии
				...
				отступ := 3
				// Подбираем количество в ряду под ширину терминала
				вРяд := 3
				maxTermWidth := 120
