...
func main() {
	...
	// Канал для отправки объектов (игр, игроков, ходов) в хранилище
	шина := make(chan model.ОбъектХранилища, 100)
	defer close(шина)

	// Горутина-читатель: принимает объекты из канала и сохраняет их в хранилище
	go func() {
	...
	// Запускаем логгер в фоновой горутине: он следит за изменениями в хранилище
	...
	go func() {
		for {
			// Поскольку на автоход требуется 2-4 секунды, плюс время ожидания,
			// я решил, что рисовать каждую секунду, как того требует задание,
